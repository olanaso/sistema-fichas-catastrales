...
import { Circle, Hash, MapPin } from "lucide-react"
import { TipoPosicionMedidor } from "@/models/tipos"

export const columnsTipoPosicionMedidor: ColumnDef<TipoPosicionMedidor>[] = [
    {
        id: "index",
        header: "N°",
        cell: ({ row }) => {
    ...
    {
        id: "posicionmed",
        accessorKey: "posicionmed",
        header: ({ column }) => {
            return <TableHeaderColumn column={column} title="Posición Medidor" />
        },
        cell: ({ row }) => {
            return (
                <div className="text-start flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-indigo-600" />
                    {row.original.posicionmed}
                </div>
            );
        },
    },
