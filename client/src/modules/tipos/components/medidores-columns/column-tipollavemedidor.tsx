...
import { Circle, Hash, Key } from "lucide-react"
import { TipoLlaveMedidor } from "@/models/tipos"

export const columnsTipoLlaveMedidor: ColumnDef<TipoLlaveMedidor>[] = [
    {
        id: "index",
        header: "N°",
        cell: ({ row }) => {
    ...
    {
        id: "llavemed",
        accessorKey: "llavemed",
        header: ({ column }) => {
            return <TableHeaderColumn column={column} title="Código Llave" />
        },
        cell: ({ row }) => {
            return (
                <div className="text-start flex items-center gap-2">
                    <Key className="w-4 h-4 text-amber-600" />
                    {row.original.llavemed}
                </div>
            );
        },
    },
