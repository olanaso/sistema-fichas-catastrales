...
import { Circle, Hash, Droplets } from "lucide-react"
import { TipoFugas } from "@/models/tipos"

export const columnsTipoFugas: ColumnDef<TipoFugas>[] = [
  {
    id: "index",
    header: "N°",
    cell: ({ row }) => {
  ...
  {
    id: "tipofugas",
    accessorKey: "tipofugas",
    header: ({ column }) => {
      return <TableHeaderColumn column={column} title="Código Fuga" />
    },
    cell: ({ row }) => {
      return (
        <div className="text-start flex items-center gap-2">
          <Droplets className="w-4 h-4 text-blue-600" />
          {row.original.tipofugas}
        </div>
      );
    },
  },
