...
import { Circle, Hash, ShieldCheck } from "lucide-react"
import { TipoEstadoTapa } from "@/models/tipos"

export const columnsTipoEstadoTapa: ColumnDef<TipoEstadoTapa>[] = [
    {
        id: "index",
        header: "N°",
        cell: ({ row }) => {
    ...
    },
    {
        ...
        accessorKey: "esttapa",
        header: ({ column }) => {
            return <TableHeaderColumn column={column} title="Estado Tapa" />
        },
        cell: ({ row }) => {
            return (
                <div className="text-start flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-green-600" />
                    {row.original.esttapa}
                </div>
            );
        },
    },
