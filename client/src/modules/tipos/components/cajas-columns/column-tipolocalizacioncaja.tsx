...
import { TipoLocalizacionCaja } from "@/models/tipos"

export const columnsTipoLocalizacionCaja: ColumnDef<TipoLocalizacionCaja>[] = [
  {
    id: "index",
    header: "N°",
    cell: ({ row }) => {
  ...
  },
  {
    ...
    accessorKey: "loccaja",
    header: ({ column }) => {
      return <TableHeaderColumn column={column} title="Localización" />
    },
    cell: ({ row }) => {
      return (
        <div className="text-start flex items-center gap-2">
          <MapPin className="w-4 h-4 text-purple-600" />
          {row.original.loccaja}
        </div>
      );
    },
  },
