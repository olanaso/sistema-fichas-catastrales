...
export const TimeInputControlled = forwardRef<HTMLInputElement, TimeInputProps>(
  (
    {
      format = "12",
      showSeconds = false,
    ...
    const { time: initialTime, amPm: initialAmPm } = extractAmPmFromValue(value as string)
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Llamar al onChange externo con el valor completo
      if (onChange) {
    ...
    const handleAmPmToggle = () => {
      ...
      if (onChange && initialTime) {
        const finalValue = `${initialTime} ${newAmPm}`
        const syntheticEvent = {
    ...
    const displayValue = initialTime
    const hasError = error || (!isValid && initialTime.length >= 5)
...
TimeInputControlled.displayName = "TimeInputControlled"
